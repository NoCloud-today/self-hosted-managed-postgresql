...
from src.api.models import Backup
...
class BackupService:
    ...
    def create_incremental_backup(self) -> str:
        return self.run_command(["/app/scripts/backup_incr.sh"], cwd="/app/scripts")

    def create_full_backup(self) -> str:
        return self.run_command(["/app/scripts/backup_full.sh"], cwd="/app/scripts")

    def create_diff_backup(self) -> str:
        return self. run_command(["/app/scripts/backup_diff.sh"], cwd="/app/scripts")

    def list_backups(self) -> List[Backup]:
        result = self.run_command(["/app/scripts/backup_info.sh"], cwd="/app/scripts")
    ...
    def restore_backup_by_time(self, timestamp: int) -> str:
        ...
        return self.run_command(
            ["/app/scripts/restore_time.sh", iso_time],
            cwd="/app/scripts"
        )

    def restore_backup_immediate(self, database_name: str = None) -> str:
        command = ["/app/scripts/restore_immediate.sh"]
        if database_name:
            command.append(database_name)
        return self.run_command(command, cwd="/app/scripts") 